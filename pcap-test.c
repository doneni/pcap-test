...
int main(int argc, char* argv[]) {
	...
	// Parsing payload up to 10 bytes
	u_int tcp_hdr_len = (tcp_hdr->th_flags >> 4) * 4;
        const u_char *payload = packet + sizeof(struct libnet_ethernet_hdr) + sizeof(struct libnet_ipv4_hdr) + tcp_hdr_len;
        u_int payload_len = header->caplen - (sizeof(struct libnet_ethernet_hdr) + sizeof(struct libnet_ipv4_hdr) + tcp_hdr_len);
        if (payload_len > 0) {
            printf("payload: ");
            for (u_int i = 0; i < payload_len && i < PAYLOAD_MAX_LEN; ++i) {
